...
export default function Header() {
  ...
  return (
    <>
                ...
                link.children ? (
                  <div key={link.href} className="relative group">
                    <button className={`flex items-center gap-1 text-sm font-medium transition-colors ${
                      pathname.startsWith(link.href) ? "text-black border-b-2 border-black pb-0.5" : "text-gray-700 hover:text-black"
                    }`}>
                      {link.label}
                      <svg
                  ...
                  <Link
                    ...
                    className={`text-sm font-medium transition-colors ${
                      pathname === link.href ? "text-black border-b-2 border-black pb-0.5" : "text-gray-700 hover:text-black"
                    }`}
                  >
                    {link.label}
                  </Link>
      ...
      {/* Mobile backdrop - outside header to avoid stacking context */}
      <div
        className={`fixed inset-0 z-[55] bg-black/40 md:hidden transition-opacity duration-300 ${
      ...
      {/* Mobile menu drawer - outside header to avoid stacking context */}
      <div
        className={`fixed inset-y-0 right-0 z-[60] w-[85vw] max-w-sm bg-white shadow-xl transition-transform duration-300 ease-in-out md:hidden ${
              ...
              <div key={link.href}>
                <Link
                  ...
                  className={`block py-3 text-base font-medium ${
                    pathname.startsWith(link.href) ? "text-black" : "text-gray-800 hover:text-black"
                  }`}
                >
                  {link.label}
                </Link>
                  ...
                  {link.children.map((child) => (
                    <Link
                      ...
                      className={`block py-2 text-sm ${
                        pathname === child.href ? "text-black font-semibold" : "text-gray-600 hover:text-black"
                      }`}
                      onClick={closeMobile}
                    >
                      {child.label}
